  ...
  },
  {
    ...
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className="w-3">
        <path fill="#ffffff" d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8l164.9-188.5L26.8 48h145.6l100.5 132.9L389.2 48zm-24.8 373.8h39.1L151.1 88h-42l255.3 333.8z"></path>
      </svg>
    ),
  },
];
...
const Footer = () => {
  return (
    <div className="px-6 sm:px-6 md:px-8 lg:px-10 xxl:px-24 bg-[#1e1519] dark:bg-[#1e1519]">
      <footer className="py-6 md:py-9 pt-12 lg:pt-24 bg-[#1e1519] dark:bg-[#1e1519] w-[100%] rounded-[5px]">
              ...
              <div className="flex flex-col justify-between h-full">
                  ...
                  <div className="flex flex-wrap items-end py-2 mt-6 border-l-2 sm:space-x-4 xs:space-x-3 sm:justify-center sm:border-l-0 sm:mt-4 md:justify-start md:border-l-2 xl:mt-8 border-red-accent">
                    {[
                      ...
                      <div key={s.val} className="px-3 xl:px-4 2xl:px-5">
                        <h3 className="font-heading text-xl xl:text-2xl font-semibold text-left 2xl:text-[1.625rem] text-white">{s.val}</h3>
                        <p className="mt-0 text-sm text-left text-white xl:text-base">{s.label}</p>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="mt-10 md:mt-12 lg:mt-20 2xl:mt-28 w-full max-w-[420px] sm:max-w-[350px] md:max-w-[400px] md:w-auto lg:max-w-[450px] sm:mx-auto md:mx-0">
                  <div className="flex items-center w-full mt-2">
                    <input
                      type="email"
                      className="2xl:h-[50px] !leading-[0.9] mr-2 px-4 py-3 md:py-3.5 lg:py-4 text-sm xl:text-sm 2xl:text-base flex-1 rounded-xl xl:rounded-[0.875rem] placeholder:text-white/50 text-white transition-all duration-300 outline-transparent focus:outline-red-accent bg-transparent border border-white border-opacity-[33%]"
                      placeholder="Enter your email address"
                    />
                    <button type="button" className="h-10 px-6 text-base min-w-fit cursor-pointer border transition-color duration-300 focus:outline-none focus:ring-0 flex items-center justify-center font-semibold primary-btn maroon no-margin !border-0 !py-4 md:!py-[1.35rem] lg:!py-6">
                      <span className="flex items-center font-heading flex-shrink-0 text-xs text-white leading-[0.813rem] sm:text-sm 2xl:text-base font-semibold inline-flex items-center">
                        Sign up <span className="icon -mt-[2px]"><ArrowIcon /></span>
                      </span>
                    </button>
                  </div>
                </div>
                ...
                <div className="flex flex-col justify-between flex-1 h-full sm:items-center md:items-start sm:flex-col md:flex-col">
                  <div className="flex flex-wrap sm:justify-center sm:w-2/3 md:justify-start md:w-full">
                    <div className="mt-10 mr-12 lg:mr-20 sm:text-center xl:mr-28 2xl:mr-40 md:mt-0 md:text-left md:w-auto">
                      <h3 className="font-heading text-lg font-bold text-white 2xl:text-xl">Courses</h3>
                      <ul className="flex flex-col mt-1 ml-0 text-base lg:mt-2 2xl:text-lg">
                        {[
                          { label: "Surah", href: "/library/surah" },
                          { label: "Subject", href: "/library/series" },
                          { label: "Arabic", href: "/library/arabic" },
                          { label: "Story", href: "/library/story-night" },
                        ].map((l) => (
                          <li key={l.label} className="!my-1 lg:!my-2">
                            <a className="inline-flex text-white link-color hover-underline" href={l.href}>{l.label}</a>
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div className="hidden w-full mt-10 sm:text-center sm:w-auto md:mt-12 xl:mt-28 md:text-left lg:block">
                    <h3 className="font-heading text-lg font-bold text-white 2xl:text-xl">Follow us</h3>
                    <div className="flex mt-4 sm:justify-center md:justify-start">
                      {socialLinks.map((s, i) => (
                        <a key={i} href={s.href} rel="noopener noreferrer" target="_blank" className="inline-flex items-center justify-center mr-2 transition duration-300 border-2 border-white border-opacity-25 rounded-full 2xl:mr-3 text-accent-maroon-low w-7 h-7 sm:w-8 sm:h-8 lg:w-9 lg:h-9 link-color">
                          {s.icon}
                        </a>
                      ))}
                    </div>
        ...
        <div className="flex flex-col items-center justify-between pt-8 mt-8 border-t sm:flex-row md:pt-12 md:mt-12 lg:pt-16 lg:mt-16 border-white/10">
          <p className="text-sm text-white/60">© 2025 Al Shatibi Academy. All rights reserved.</p>
          <div className="flex gap-4 mt-4 sm:mt-0">
            <a href="#" className="text-sm text-white/60 hover:text-white">Terms &amp; Conditions</a>
